...
onMounted(() => {
  ...
  // Hiệu ứng Parallax cho ảnh Banner
  setTimeout(() => {
    if (parallaxImg.value) {
      gsap.to(parallaxImg.value, {
        yPercent: 30, // Đẩy ảnh trượt xuống 30% khi cuộn
        ease: "none",
        scrollTrigger: {
          trigger: parallaxImg.value.parentElement,
          start: "top top",
          end: "bottom top",
          scrub: true, // Đồng bộ hóa chặt chẽ với thanh cuộn
        },
      });
    }
  }, 100);
  ...
  >
    ...
    >
      <img
        ref="parallaxImg"
        :src="project.image"
        class="absolute inset-0 w-full h-[130%] object-cover object-center -top-[15%] filter brightness-[0.85]"
      />
      <div
      ...
      <div class="absolute top-10 left-6 md:left-20 z-10">
        <button
          ...
          class="group text-sm font-mono text-dark hover:text-indigo-600 flex items-center gap-3 transition-colors bg-white/90 backdrop-blur-md px-5 py-2.5 rounded-full shadow-sm hover:shadow-md"
        >
          <span
    ...
    >
      <header
        class="mb-16 bg-white/90 backdrop-blur-2xl p-8 md:p-12 rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-white"
      >
            ...
            <div class="flex flex-wrap gap-2.5">
              <template v-for="link in project.links" :key="link.label">
                <a
                  ...
                  :href="link.url"
                  target="_blank"
                  class="inline-flex items-center text-sm font-mono font-semibold px-4 py-2.5 bg-dark text-white rounded-xl shadow-sm transition-transform hover:-translate-y-1"
                >
                  {{ link.label }}
                </a>
                <span
                  v-else
                  class="inline-flex items-center text-sm font-mono font-semibold px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl text-gray-400 select-none"
                >
                  {{ link.label }}
                </span>
              </template>
      ...
      <div class="space-y-24 leading-relaxed text-lg">
        ...
        <section>
          <div class="flex items-center gap-4 mb-8">
            <div
              class="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold font-mono text-sm"
            >
              01
            </div>
          ...
          <p
            class="text-muted font-medium text-lg leading-loose pl-12 border-l-2 border-indigo-100"
          >
            {{ project.overview }}
          </p>
        </section>
        ...
        <section class="border-t border-gray-200 pt-20">
          <div class="flex items-center gap-4 mb-10">
            <div
              class="w-8 h-8 rounded-lg bg-dark flex items-center justify-center text-white font-bold font-mono text-sm"
            >
              04
            </div>
            <h2 class="text-3xl font-bold text-dark tracking-tight">
          ...
          <div class="grid md:grid-cols-2 gap-8 pl-0 md:pl-12">
            <div
              class="relative p-8 bg-white border border-red-100 rounded-3xl shadow-[0_8px_20px_rgb(220,38,38,0.04)] overflow-hidden"
            >
              <div class="absolute top-0 left-0 w-full h-1 bg-red-400"></div>
              <h3
                class="font-bold tracking-tight mb-4 flex items-center gap-3 uppercase font-mono text-xs text-red-500"
              >
                ■ Problem_Statement
              </h3>
              <p class="text-muted text-base leading-relaxed font-medium">
                {{ project.challenges }}
              </p>
            </div>

            <div
              class="relative p-8 bg-white border border-emerald-100 rounded-3xl shadow-[0_8px_20px_rgb(5,150,105,0.04)] overflow-hidden"
            >
              <div
                class="absolute top-0 left-0 w-full h-1 bg-emerald-400"
              ></div>
              <h3
                class="font-bold tracking-tight mb-4 flex items-center gap-3 uppercase font-mono text-xs text-emerald-500"
              >
